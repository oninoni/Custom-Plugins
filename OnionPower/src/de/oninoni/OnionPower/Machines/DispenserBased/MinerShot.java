...
public class MinerShot {
	...
	public boolean update() {
		...
		Collection<Entity> intheWay = position.getWorld().getNearbyEntities(position.clone(),
				direction.normalize().getX() / 2.0, direction.normalize().getY() / 2.0,
		...
		for (int i = -4; i < 4; i++) {
			// position.getWorld().spawnParticle(Particle.REDSTONE, , 1, 0, 0,
			// 0, 0);
			position.getWorld().spawnParticle(Particle.REDSTONE,
					position.clone().add(direction.clone().multiply(-i / 8.0f)), 0, 1, 0, 1, 1);
		}
		...
		if (position.clone().subtract(target.getLocation().toVector()).length() < 2) {
			...
			for (int j = 0; j < 100; j++) {
				position.getWorld().spawnParticle(Particle.REDSTONE,
						target.getLocation().clone().add(r.nextFloat(), r.nextFloat(), r.nextFloat()), 0, 1, 0, 1, 1);
			}
		}
		if (position.clone().subtract(target.getLocation().toVector()).length() < 1) {
			position.getWorld().playSound(target.getLocation().clone().add(0.5, 0.5, 0.5),
					Sound.ENTITY_ENDERMEN_TELEPORT, 0.2f, 0.8f);
			return true;
		}
		return false;
	}
