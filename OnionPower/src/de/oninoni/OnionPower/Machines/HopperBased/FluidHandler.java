			...
			if(e.getSlot() == 3 || e.getSlot() == 4){
				...
				if(e.getCurrentItem() == null || e.getCurrentItem().getType() == Material.AIR){
					...
					if(e.getCurrentItem().getType() == Material.BUCKET){
						...
						e.setCancelled(true);
					}else if(e.getCurrentItem().getType() == Material.LAVA_BUCKET || e.getCurrentItem().getType() == Material.WATER_BUCKET){
						if(e.getCursor() == null)return false;
						if(e.getCursor().getAmount() > 1){
							...
							leftOver.setAmount(leftOver.getAmount() - 1);
							e.setCancelled(true);
							HashMap<Integer, ItemStack> overflow = e.getWhoClicked().getInventory().addItem(filledBucket);
							if(overflow.size() > 0){
